...
<ul 
  <?php if ($parent) : ?> 
    ...
    x-collapse 
    class="m-0 p-0 pl-2 ml-4 w-full text-sm font-medium border-0 border-l-1 border-solid border-text-50" 
  <?php else : ?> 
  ...
  <?php foreach ($items as $item): ?>
    <li class="list-none flex justify-between flex-wrap items-center w-full <?php echo esc_attr($item->classes); ?>" x-data="{activeChild: '<?php echo $item->activeAncestor ? true : false ?>'}">
      <a
        ...
        class="block flex-1 py-2 px-4 my-2 font-primary font-semibold text-sm text-text-900 rounded-full <?php echo esc_attr($item->active ? 'bg-gray-100' : ''); ?>">
        <?php echo $item->label; ?>
      </a>

      <?php if ($item->children) : ?>
        <button class="w-8 h-8 flex justify-center items-center py-2 bg-gray-50 border-1 border-solid border-gray-300 rounded-full" x-on:click="activeChild = !activeChild" aria-label="<?php echo esc_html(__("Open", '[TEXT_DOMAIN]') . ' ' . $item->label); ?>">
