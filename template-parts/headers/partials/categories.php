...
<ul 
  ...
  ?>
    <li class="list-none flex justify-between flex-wrap items-center w-full" x-data="{activeChild: false}">
      <a href="<?php echo $item['url']; ?>" class="flex justify-start items-center flex-1 py-2 my-2 font-primary text-sm text-text-900 font-semibold rounded-full">
        <?php if($image): ?>
          <img
      ...
      <?php if (isset($item['children']) && !empty($item['children'])) : ?>
        <button class="w-8 h-8 flex justify-center items-center py-2 bg-gray-50 border-1 border-solid border-gray-300 rounded-full" x-on:click="activeChild = !activeChild" aria-label="<?php echo esc_html(__("Open", '[TEXT_DOMAIN]') . ' ' . $item['name']); ?>">
